...
visualize.overlap <- function(model.1, model.2, env, nbins = 100, layers, plot.points = TRUE){
  ...
  for(i in names(env)){
    if(!(i %in% layers)){
      layer.values <- raster::extract(env[[i]], rbind(points.1, points.2))
      plot.df <- cbind(plot.df, rep(mean(layer.values, na.rm=TRUE), nrow(plot.df)))
      names <- c(names, i)
    }
  }

  pointdata.1 <- as.data.frame(raster::extract(env[[layers]], points.1))
  pointdata.2 <- as.data.frame(raster::extract(env[[layers]], points.2))
